...
local function SpawnReincarnateNpcByID(id)
    ...
    local ok, err = pcall(function()
        ...
        -- The vanilla rename window blocks reincarnated NPCs only while
        -- ComeNpc is true. Clear it after recruitment so the original rename
        -- UI works and the chosen name is stored on the real NPC.
        pcall(function() npc.ComeNpc = false end)
...
local function AddSelectorButtonToNpc(npc)
    ...
    -- Migrate reincarnated disciples created by older selector builds.
    local isPlayer = false
    pcall(function() isPlayer = npc.Camp == g_emFightCamp.Player end)
    if not isPlayer then
        pcall(function() isPlayer = npc.OCamp == g_emFightCamp.Player end)
    end
    if isPlayer then
        pcall(function()
            if npc.ComeNpc == true then npc.ComeNpc = false end
        end)
    end
